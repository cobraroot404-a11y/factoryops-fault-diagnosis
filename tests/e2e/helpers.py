...
def send_reading(client: httpx.Client, api_key: str, ts: datetime, temperature_c=50.0, current_a=10.0, vibration_mm_s=2.0, idempotency_key=None):
    return client.post(
        "/telemetry/readings",
        headers={"X-API-Key": api_key},
        json={
            "ts": ts.isoformat(), "temperature_c": temperature_c, "current_a": current_a,
            "vibration_mm_s": vibration_mm_s, "idempotency_key": idempotency_key or str(uuid.uuid4()),
        },
    )
